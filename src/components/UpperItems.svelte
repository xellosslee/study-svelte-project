<script>
	...
	function checkboxGroupClick(e) {
		...
		chk.extra = {
			...(chk.extra || {}),
			indeterminate: chk.data.length > 0 && chk.data.length < chk.options.length,
		}
	}
</script>
	...
	<div class="inputWrap">
		{#each filterList as { align, type, label, title, html, field, func, data, options, required, extra, ...attrs }}
			{#if align == undefined || align == 'left'}
				<div class="upperItem">
					{#if type == 'text'}
						<Text bind:value={data} {label} {title} {required} {...attrs} />
					{:else if type == 'group-radio'}
						<Radio bind:value={data} {label} {title} {options} on:change={groupChange} />
					{:else if type == 'radio'}
						<Radio bind:value={data} {label} {title} {options} on:change={func} />
					{:else if type == 'date'}
						<Datefield {label} value={data} format="YYYY-MM-DD" message="YYYY-MM-DD" on:date-change={func} {required} />
